...
pub trait Biome: FromRng + Send + Sync {
    ...
    /// Registers the structures that originate from the provided chunk.
...
pub struct BiomeRegistry {
    /// The maximum size of the structures that can be spawn in any biome.
    max_structure_size: i32,
    /// The created biomes.
    biomes: [BiomeInfo; BiomeId::COUNT],
}

impl BiomeRegistry {
    /// Returns the maximum size of the structures that can be spawn in any biome.
    #[inline]
    pub fn max_structure_size(&self) -> i32 {
...
impl FromRng for BiomeRegistry {
    fn from_rng(rng: &mut impl Rng) -> Self {
        ...
        Self {
            max_structure_size: biomes.iter().fold(0, |a, b| a.max(b.max_structure_size)),
            biomes,
        }
    }
}
